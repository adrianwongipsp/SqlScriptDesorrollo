...
--select * from  TRANSFERENCIAS WHERE Cod_Ciclo_Origen = @PISCINALLAVE
	...
	select pu.codigoZona, pu.nombreZona, pu.codigoCamaronera, pu.nombreCamaronera, pu.codigoSector, pu.nombreSector, pu.codigoPiscina, pu.nombrePiscina, ce.codigoElemento, ce.nombreElemento, pe.idPiscina, 
	...
	select 
		ROW_NUMBER ( ) OVER ( order by t.fecha_Transf, Sector_Origen + CONVERT(VARCHAR(200),Piscina_Origen), t.Cod_Ciclo_Origen)  as rowKeySecuencia#,
		...
		Sector_Origen + CONVERT(VARCHAR(200),Piscina_Origen) as keyOrigen, Cod_Ciclo_Origen, fecha_Transf, Zona_Origen, Camaronera_Origen, Sector_Origen, Piscina_Origen, Ciclo_Origen, Tipo,	
		Sub_Tipo, Estatus_Origen, Hectarea_Origen, Fecha_Siembra, Cantidad_Sembrada, Densidad, Peso_Siembra, Estatus_Transf#, 
		Cons_Balanc#, Guia_Transf#, Forma_Transf#,   /*CORREGIR NO ES LINEA ES TIPOTRANSFERNEICA*/
		...
		Sector_Destino + CONVERT(VARCHAR(200),Piscina_Destino) as keyDestino, Cod_Ciclo_Destino, puD.codigoZona as Zona_Destino, puD.codigoCamaronera as  Camaronera_Destino, puD.codigoSector as  Sector_Destino, 
	...
	from TRANSFERENCIAS t
		INNER join #EJECUCIONESTEMP pu on pu.Cod_Ciclo = t.Cod_Ciclo_Origen
		INNER join #EJECUCIONESTEMP puD on puD.Cod_Ciclo = t.Cod_Ciclo_Destino
		WHERE t.Cod_Ciclo_Origen = @PISCINALLAVE
	order by 1
	
	select 
		ROW_NUMBER ( ) OVER ( order by r.Fecha_Siembra, r.[Key], r.Cod_Ciclo)  as rowKeySecuencia#,
		ROW_NUMBER ( ) OVER ( PARTITION BY r.[Key] order by r.Fecha_Siembra, r.[Key], r.Cod_Ciclo)  as rowKey#, r.[Key] as KeyPiscina,
		ROW_NUMBER ( ) OVER ( PARTITION BY r.Cod_Ciclo order by r.Fecha_Siembra, r.[Key], r.Cod_Ciclo)  as rowCiclo#, r.Cod_Ciclo,
		0 as idRecepcion,  '01' as empresa, '01' as division, r.Zona as NombreZoma, pu.codigoZona as zona, r.Camaronera as nombreCamaronera, pu.codigoCamaronera as camaronera, r.Sector as nombreSector, pu.codigoSector as sector, 
		...
		0 as idLaboratorio, 0 as idLaboratorioLarva,  0 as idLaboratorioMaduracion, Peso_Siembra, [Pl/Gr_Guía] as plGramoLab, [Pl/Gr_Llegada] as plGramoCam,
		...
		r.Guia_Remisión as guiasRemision, 0 as tieneFactura, 'Regularizada' as descripcion, '' as responsableEntrega, 'APR' AS estado, 'AdminPsCam' as usuarioResponsable,
	...
	from RECEPCIONES r INNER join #EJECUCIONESTEMP pu on pu.Cod_Ciclo = r.Cod_Ciclo 
	where r.Cod_Ciclo = @PISCINALLAVE
	order by 1
	...
	BEGIN TRY
		begin tran
							...
							--select top 4 'antes', * from proRecepcionEspecie order by 2 desc
							--select top 4 'antes', * from proRecepcionEspecieDetalle order by 2 desc
							select top 4 'antes', * from proPiscinaEjecucion order by 2 desc
							select top 4 'antes', * from maePiscinaCiclo order by 2 desc
				...
				select top 1 @cicloRecepcion = Cod_Ciclo from #RecepcionesTemp  where procesado = 0 order by rowKeySecuencia#;
				...
				SELECT
					ROW_NUMBER() OVER(ORDER BY Cod_Ciclo) + @secuencialTransferencia AS idTransferencia,
					ROW_NUMBER() OVER(ORDER BY Cod_Ciclo) + @secuencialTransferenciaDetalle AS idTransferenciaDetalle,
					empresa, fecha_Transf AS fechaRegistro, fecha_Transf as fechaTransferencia, ROL, Cod_Ciclo_Origen,
					tipoTransferencia, idPiscina, 1 as idEspecie, 0 as guiaTransferencia, Cantidad_Transf# as cantidadTransferida,
					Peso_Transf# as pesoTransferidoGramos, @secuencialPiscinaEjecucion as idPiscinaEjecucion, 0 AS esTotal,  Sector_Destino,
					...
					Peso_Siembra, Peso_Transf#, Peso_Real, Lib_Transf_C, Libras_Transf# as librasDeclaradas, Ciclo_Destino, Sector_Destino
					fechaHoraModificacion, null as reprocesoContable, Cod_Ciclo_Destino
				...
				SELECT
					...
					CASE WHEN ROW_NUMBER() OVER (PARTITION BY fecha_Transf ORDER BY 1 DESC) = 1 THEN 'TRUE' ELSE 'FALSE' END AS esTotal, 
					cantidadRemanente, densidad, balanceado, descripcion, usuarioResponsable, estado, usuarioCreacion, 
					estacionCreacion, fechaHoraCreacion, usuarioModificacion, estacionModificacion, 
					fechaHoraModificacion, reprocesoContable
				FROM #InsertTransfTemp;

				SELECT @cantidadTransferencia = SUM(cantidadTransferida) FROM #InsertTransfTemp
				SELECT @FechaCierre = max(fechaTransferencia) FROM #InsertTransfTemp
				...
				INSERT INTO [dbo].[proTransferenciaEspecieDetalle] (
					...
					numeroViajes)
				SELECT
					idTransferenciaDetalle, idTransferencia, 1, idPiscina, ROL as rolPiscina, cantidadTransferida,
				...
				FROM #InsertTransfTemp;
					...
					INSERT INTO proPiscinaEjecucion (
							...
							tieneRepanio, tieneCosecha, activo, usuarioCreacion, estacionCreacion, fechaHoraCreacion, 
							usuarioModificacion, estacionModificacion, fechaHoraModificacion)
					select @secuencialPiscinaEjecucion, idPiscina, Ciclo_Destino, ROL, 1, Sector_Destino, DATEADD(DAY, -1, min(fechaTransferencia)), min(fechaTransferencia), null, '', 1, 
							SUM(cantidadTransferida), 0, 0, 0, @secuencialPiscinaEjecucionSiguiente, 'EJE', 0, 0, 0, 1,
							@usuarioAudit, '::2', @fechaProceso,@usuarioAudit, '::2', @fechaProceso
					from #InsertTransfTemp 
					group by idPiscina, Cod_Ciclo_Destino, rol, Sector_Destino
					...
					UPDATE proSecuencial 
							...
							--select top 4 'despues', * from proRecepcionEspecie order by 2 desc
							--select top 4 'despues', * from proRecepcionEspecieDetalle order by 2 desc
							select top 4 'despues', * from proPiscinaEjecucion order by 2 desc
							select top 4 'despues', * from maePiscinaCiclo order by 2 desc
