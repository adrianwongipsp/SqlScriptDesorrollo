...
DECLARE @KEY_PISCINA VARCHAR(100) = 'CAPULSAPC2'

--alter table RECEPCIONES_PRODUCCION
--add procesado bit

	drop table if exists #QUERYCICLOSFALTANTES
	drop table if exists #FILTROPISCINA			
	...
	select distinct Key_Piscina into #FILTROPISCINA from CICLOS_PRODUCCION where Sector = 'CAPULSA'
	...
	pe.idPiscinaEjecucion, pe.ciclo, pe.fechaInicio, pe.fechaSiembra, pe.fechaCierre, pe.cantidadEntrada,
	...
	--SCRIPT DIFERENCIA DE DIAS Y CICLOS FALTANTES
	...
	ej.fechaInicio, ej.fechaSiembra, ej.fechaCierre, ej.cantidadEntrada, ej.ciclo, ej.idPiscina, ej.idPiscinaEjecucion, DATEDIFF(day, Fecha_IniSec, ej.fechaInicio) InicioDiff, DATEDIFF(day, Fecha_Siembra, ej.fechaSiembra) SiembraDiff,DATEDIFF(day, Fecha_Pesca, ej.fechaCierre) CierreDiff,
	(Cantidad_Sembrada - ej.cantidadEntrada) CantidadDiff
	INTO #QUERYCICLOSFALTANTES
	from CICLOS_PRODUCCION cp
	left JOIN #EJECUCIONESTEMP ej on cp.Cod_Ciclo = ej.Cod_Ciclo 
	where Key_Piscina IN (SELECT Key_Piscina FROM #FILTROPISCINA) and Estatus = 'ACTIVO' AND cp.ciclo >= (select MIN(Ciclo) from #EJECUCIONESTEMP where keyPiscina = CP.Key_Piscina) 
	ORDER BY 1 
	...
	from RECEPCIONES_PRODUCCION r left join #EJECUCIONESTEMP pu on pu.Cod_Ciclo = r.Cod_Ciclo order by 1

	SELECT * FROM #RecepcionesTemp rt
	INNER JOIN proRecepcionEspecieDetalle re on rt.idPiscina = re.idPiscina

	select distinct rt.Cod_Ciclo, rt.Ciclo, rt.cantidad, rt.porcentajePlus, rt.fechaRecepcion, tp.idPiscina, tp.idPiscinaEjecucion, tp.keyPiscina, tp.cod_ciclo, tp.fechaRecepcion, tp.idRecepcion, tp.cantidadRecibida, cantidadAdicional
	from #RecepcionesTemp rt
		left join (select ej.keyPiscina, ej.cod_ciclo, ej.idPiscina, ej.idPiscinaEjecucion, r.fechaRecepcion, r.idRecepcion, re.cantidadRecibida, cantidadAdicional
		 from proRecepcionEspecieDetalle re 
		inner join EjecucionesPiscinaView ej on re.idPiscinaEjecucion = ej.idPiscinaEjecucion
		inner join proRecepcionEspecie r on r.idRecepcion = re.idRecepcion ) tp on  rt.Cod_Ciclo = tp.Cod_Ciclo
		where rt.Cod_Ciclo in ('CAPULSAPC2.91')
		--where rt.KeyPiscina in (select * from #FILTROPISCINA)

		
	select distinct tt.Cod_Ciclo_Origen, tt.Ciclo_Origen, tt.Cantidad_Transf, tt.Fecha_Transf, tp.idPiscina, tp.idPiscinaEjecucion, tp.keyPiscina, tp.cod_ciclo, tp.fechaTransferencia, tp.idTransferencia, tp.cantidadTransferida
	from #TranferenciaTemp tt 
		left join (select ej.keyPiscina, ej.cod_ciclo, ej.idPiscina, ej.idPiscinaEjecucion, t.fechaTransferencia, t.idTransferencia, t.cantidadTransferida
		 from proTransferenciaEspecie t 
		inner join EjecucionesPiscinaView ej on t.idPiscinaEjecucion = ej.idPiscinaEjecucion ) tp on  tt.Cod_Ciclo_Origen = tp.Cod_Ciclo
		--where tt.keyOrigen in (select * from #FILTROPISCINA)
		where tt.keyOrigen in ('CAPULSAPC2')
	...
	declare @usuarioAudit varchar(50) = 'AdminPsCam'
			...
			BEGIN
				...
				--COMPRUEBO SI YA CONTIENE IDPISCINA EJECUCIÓN
				IF (select COUNT(1) from EjecucionesPiscinaView where cod_ciclo = @cicloRecepcion) >0
					BEGIN --yo se que aun falta el cilco 0 y cuando vienen en estado iniciado
						select @secuencialPiscinaEjecucion = idPiscinaEjecucion from EjecucionesPiscinaView where cod_ciclo = @cicloRecepcion
					END
				ELSE
					Begin
					...
					--VERIFICO SI YA EXISTE LA RECEPCIÓN PARA ESA EJECUCIÓN
				...
				select distinct rt.Cod_Ciclo, rt.Ciclo, rt.cantidad, rt.porcentajePlus, rt.fechaRecepcion, tp.idPiscina, tp.idPiscinaEjecucion, tp.keyPiscina, tp.cod_ciclo, tp.fechaRecepcion,
				tp.idRecepcion, tp.cantidadRecibida, cantidadAdicional
				from #RecepcionesTemp rt
					left join (select ej.keyPiscina, ej.cod_ciclo, ej.idPiscina, ej.idPiscinaEjecucion, r.fechaRecepcion, r.idRecepcion, re.cantidadRecibida, cantidadAdicional
					 from proRecepcionEspecieDetalle re 
					inner join EjecucionesPiscinaView ej on re.idPiscinaEjecucion = ej.idPiscinaEjecucion
					inner join proRecepcionEspecie r on r.idRecepcion = re.idRecepcion ) tp on  rt.Cod_Ciclo = tp.Cod_Ciclo
					where rt.Cod_Ciclo = 'CAPULSAPC2.89'--@cicloRecepcion
					--where rt.KeyPiscina in (select * from #FILTROPISCINA)
