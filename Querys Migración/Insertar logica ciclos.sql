...
--add procesado bit
	...
	select 
		...
		Sector_Destino + CONVERT(VARCHAR(200),Piscina_Destino) as keyDestino, Cod_Ciclo_Destino, puD.codigoZona as Zona_Destino, puD.codigoCamaronera as  Camaronera_Destino, puD.codigoSector as  Sector_Destino, Piscina_Destino, Ciclo_Destino, Estatus_Destino, Hectarea_Destino, Cantidad_Transf, Peso_Transf, Peso_Real,
	...
	from TRANSFERENCIAS_PRODUCCION t
		...
		INNER join #EJECUCIONESTEMP puD on pu.Cod_Ciclo = t.Cod_Ciclo_Destino
	order by 1
	
	select 
		ROW_NUMBER ( ) OVER ( order by r.Fecha_Siembra, r.[Key], r.Cod_Ciclo)  as rowKeySecuencia#,
		ROW_NUMBER ( ) OVER ( PARTITION BY r.[Key] order by r.Fecha_Siembra, r.KeyPiscina, r.Cod_Ciclo)  as rowKey#, r.KeyPiscina as KeyPiscina,
		...
		begin tran
			...
			BEGIN
				...
				select @secuencialPiscinaEjecucion, idPiscina, Ciclo, ROL, 1, sector, DATEADD(DAY, -1, min(fechaRecepcion)), min(fechaRecepcion), null, '', 1, 
						SUM(cantidad),SUM( cantidadPlus), 0, 0, @secuencialPiscinaEjecucionSiguiente, 'EJE', 0,0,0,1,
				...
				FROM #InsertRecepTemp
					...
					SELECT
						...
						ROL as rolPiscina, cantidad, unidadMedida, cantidadRecibida, cantidadPlus,
				...
				SELECT
					...
					CASE WHEN ROW_NUMBER() OVER (PARTITION BY fechaTransferencia ORDER BY 1 DESC) = 1 THEN 'TRUE' ELSE 'FALSE' END AS esTotal, 
