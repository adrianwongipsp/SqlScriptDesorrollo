   ...
   --STEP 2: Query para creación de tabla de transferencia con homologaciones de piscinas con códigos de Insigne
	...
	FROM TRANSFERENCIAS_PRODUCCION t   
		left join #EJECUCIONESTEMP pu  ON pu.keyPiscina  = REPLACE(t.keyPiscinaOrigen ,' ','')   
		LEFT join #EJECUCIONESTEMP puD ON puD.keyPiscina = REPLACE(t.keyPiscinaDestino ,' ','')
		left join PESCA_PRODUCCION pp ON pp.Cod_ciclo = t.Cod_Ciclo_Destino
	...
	BEGIN TRY
		...
		SELECT top 1 1 FROM maeSecuencial WITH (TABLOCKX)
			...
			select @cpount as contadorDeVueltas
				...
				--SET @cicloTransferencia = @PISCINALLAVE --select top 1 @cicloTransferencia = @PISCINALLAVE from ##TranferenciaTemp  where procesado = 0 order by rowKeySecuencia#;
				...
				SELECT @secuencialPiscinaEjecucion = idPiscinaEjecucion FROM EjecucionesPiscinaView WHERE cod_ciclo = @cicloTransferencia
				--SELECT * FROM EjecucionesPiscinaView WHERE cod_ciclo = @cicloTransferencia
				select @cicloTransferencia
				...
				select 'EJEMPLO DE TRANS VALID', T.* from proTransferenciaEspecieDetalle TE
						...
						inner join EjecucionesPiscinaView ej on ej.idPiscinaEjecucion = tE.idPiscinaEjecucion 
								where ej.cod_ciclo = @cicloTransferencia AND T.estado = 'APR'

				if not exists ( select 1 from proTransferenciaEspecieDetalle  TE
						INNER JOIN proTransferenciaEspecie T ON TE.idTransferencia = T.idTransferencia 
						inner join EjecucionesPiscinaView ej on ej.idPiscinaEjecucion = tE.idPiscinaEjecucion 
								where ej.cod_ciclo = @cicloTransferencia AND T.estado = 'APR')
					BEGIN
						...
						select 'trans'
								...
								if @estadoTransferencia = 'TOTAL'
									BEGIN
										...
										SELECT @secuencialPiscinaEjecucionSiguiente AS SDASDSA
