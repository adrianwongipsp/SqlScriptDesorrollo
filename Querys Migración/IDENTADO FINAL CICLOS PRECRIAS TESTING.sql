	...
	DECLARE @PISCINALLAVE VARCHAR(100) = 'CAPULSAPC4'
	DECLARE @SECTORFILTER VARCHAR(100) = 'COSTARICAI'
	...
	--WHERE cp.ciclo >= (select min(ciclo) from #EJECUCIONESTEMP p where p.keyPiscina = cp.Key_Piscina) AND REPLACE(Sector,' ','') = @SECTORFILTER order by 1 -- @SECTORFILTER --and ej.ciclo is null
   ...
   --FILTRAMOS LAS PISCINAS A REGULARIZAR
		...
		from CICLOS_PRODUCCION cp
		left JOIN #EJECUCIONESTEMP ej on cp.Cod_Ciclo = ej.Cod_Ciclo 
		WHERE cp.ciclo >= (select min(ciclo) from #EJECUCIONESTEMP p where p.keyPiscina = cp.Key_Piscina) AND REPLACE(Sector,' ','') = @SECTORFILTER and ej.ciclo is null
   ...
   --STEP 2: Query para creación de tabla de transferencia con homologaciones de piscinas con códigos de Insigne
	...
	WHERE T.keyPiscinaOrigen IN (SELECT FTP.Key_Piscina FROM #FILTERPISCINAKEYTEMP FTP) --Sector_Origen = @SECTORFILTER
	ORDER BY 1
	...
	SELECT 	ROW_NUMBER ( ) OVER ( order by t.fecha_Transf,  t.keyOrigen,  t.keyDestino)  as rowKeySecuencia#,
	 ...
	 --STEP 3: Query para creación de tabla de recepciones con homologaciones de piscinas con códigos de Insignenote
	SELECT 
		...
		0 procesado,
		(select  top 1 cp.Cod_Ciclo from CICLOS_PRODUCCION cp where cp.Key_Piscina = pu.KeyPiscina  and pu.fechaSiembra between cp.Fecha_Siembra and cp.Fecha_Pesca) as  Cod_Ciclo_Real
	...
	WHERE R.KeyPiscina IN (SELECT FTP.Key_Piscina FROM #FILTERPISCINAKEYTEMP FTP) --r.Sector = @SECTORFILTER
	ORDER BY 1;
	...
	BEGIN TRY
		...
		declare @secuencialMaeCicloEXISTENTE int = 0
		...
		SELECT top 1 1 FROM maeSecuencial WITH (TABLOCKX)
				...
				select 'antes propiscinaejecucion',* from EjecucionesPiscinaView where nombreSector = @SECTORFILTER and estado in ('INI','EJE', 'PRE') ORDER BY 2,5
			...
			BEGIN
				...
				IF (select COUNT(1) from EjecucionesPiscinaView where cod_ciclo = @cicloRecepcion) >0
					BEGIN --yo se que aun falta el cilco 0 y cuando vienen en estado iniciado
						...
						select @secuencialMaeCicloEXISTENTE = idPiscinaCiclo from maePiscinaCiclo where idOrigen = @secuencialPiscinaEjecucion
						
						SELECT TOP 1 @secuencialMaeCiclo = ultimaSecuencia + 1
						FROM	maeSecuencial 
						WHERE	tabla = 'PiscinaCiclo'

						if @secuencialPiscinaEjecucionSiguiente is null
							SELECT TOP 1 @secuencialPiscinaEjecucionSiguiente = ultimaSecuencia + 1
							FROM	proSecuencial 
							WHERE	tabla = 'piscinaEjecucion' 

						select 1 AS CONDICION, @cicloRecepcion AS CICLO, @secuencialPiscinaEjecucion id, @secuencialPiscinaEjecucionSiguiente idSgte
				...
				ELSE IF EXISTS (SELECT 1 FROM CICLOS_PRODUCCION CP INNER JOIN EjecucionesPiscinaView E ON E.keyPiscina = CP.Key_Piscina
					...
					BEGIN
						SELECT TOP 1  @secuencialPiscinaEjecucion = E.idPiscinaEjecucion
							...
							select 2 AS CONDICION, @cicloRecepcion AS CICLO, @secuencialPiscinaEjecucion id, @secuencialPiscinaEjecucionSiguiente idSgte 
				...
				ELSE
					...
					select 3 AS CONDICION, @cicloRecepcion AS CICLO, @secuencialPiscinaEjecucion id, @secuencialPiscinaEjecucionSiguiente idSgte 
				...
				IF exists ( select top 1 1 from proRecepcionEspecieDetalle re 
								...
								inner join CICLOS_PRODUCCION cp on cp.Cod_Ciclo = ej.Cod_Ciclo
								where ej.Cod_Ciclo = 'COSTARICAI10.69' /*@cicloRecepcion*/ 
								AND CP.[Fecha_Siembra] <= DATEADD(DAY, 0, r.fechaRecepcion)   
								AND DATEADD(DAY, 0,  r.fechaRecepcion)  >= CP.[Fecha_Pesca] )
				BEGIN
					select 'validación de recepcion 1'
					--select Cod_Ciclo_Real, fechaRecepcion, cantidadRecibida
					--from #RecepcionesTemp where Cod_Ciclo = 'COSTARICAI10.69' /*@cicloRecepcion*/ 

					--select * from proRecepcionEspecieDetalle re 
					--inner join proRecepcionEspecie r on re.idRecepcion = r.idRecepcion
					--inner join CICLOS_PRODUCCION cp on cp.Fecha_Siembra = r.fechaRecepcion 
					
					DECLARE @idPiscinaDESMONTAR int, @idPiscinaEjecucionDESMONTAR int;

					--select  R.fechaRecepcion, CP.[Fecha_Siembra], CP.[Fecha_Pesca],ej.cod_ciclo cicloRecepcion, cp.Cod_Ciclo cicloArchivo, eja.idPiscinaEjecucion as idPiscinaEjecucionArchivo, re.idPiscinaEjecucion,
					--re.*
					--from proRecepcionEspecieDetalle re 
					--				inner join proRecepcionEspecie r on re.idRecepcion = r.idRecepcion
					--				inner join EjecucionesPiscinaView ej on re.idPiscinaEjecucion = ej.idPiscinaEjecucion
					--				inner join CICLOS_PRODUCCION cp on cp.Fecha_Siembra = r.fechaRecepcion and cp.Key_Piscina = ej.keyPiscina
					--				inner join EjecucionesPiscinaView eja on eja.cod_ciclo = cp.Cod_Ciclo
					--				--inner join #RecepcionesTemp rt on 
					--				where ej.Cod_Ciclo = 'COSTARICAI10.69' /*@cicloRecepcion*/ 

					update re
					set re.idPiscinaEjecucion = eja.idPiscinaEjecucion
					--select  R.fechaRecepcion, CP.[Fecha_Siembra], CP.[Fecha_Pesca],ej.cod_ciclo cicloRecepcion, cp.Cod_Ciclo cicloArchivo, eja.idPiscinaEjecucion as idPiscinaEjecucionArchivo, re.idPiscinaEjecucion,
					--re.*
					from proRecepcionEspecieDetalle re 
									inner join proRecepcionEspecie r on re.idRecepcion = r.idRecepcion
									...
									inner join EjecucionesPiscinaView eja on eja.cod_ciclo = cp.Cod_Ciclo
									where ej.Cod_Ciclo = 'COSTARICAI10.69' /*@cicloRecepcion*/ 
									and eja.idPiscinaEjecucion != re.idPiscinaEjecucion

					--ACTUALIZAMOS de ciclo montado
					delete from ciclo_regularizar

					exec USP_REGULARIZACIONCICLOSEXCEL @keyPiscina

					UPDATE PE
						SET 
							PE.fechaInicio		= cr.fechaInicioOrigen,
							PE.fechaSiembra		= cr.fechaSiembraOrigen,
							PE.fechaCierre		= cr.fechaPescaOrigen,
							PE.cantidadEntrada		= cr.cantidadSembradaOrigen,
							--PE.cantidadSalida		= @idPiscinaEjecucionDESMONTAR,
							PE.cantidadAdicional		= cr.cantidadSembradaOrigen + (cr.cantidadSembradaOrigen * 0.15),
							--cantidadSalida				= @cantidadTransferencia,
							--cantidadPerdida				= (cantidadEntrada + cantidadAdicional) - @cantidadTransferencia,
							PE.estado						= cr.estado,
							PE.ciclo					= CR.ciclo
						FROM proPiscinaEjecucion PE 
						inner join ciclo_regularizar cr on pe.idPiscinaEjecucion = cr.idPiscinaEjecucion;



						--DROP TABLE IF EXISTS #IDSRECEPCIONTEMP

						--select re.idRecepcion, re.idRecepcionDetalle, r.fechaRecepcion, re.idPiscina, re.idPiscinaEjecucion
						--INTO #IDSRECEPCIONTEMP
						--from proRecepcionEspecieDetalle re 
						--	inner join proRecepcionEspecie r on re.idRecepcion = r.idRecepcion
						--	inner join EjecucionesPiscinaView ej on re.idPiscinaEjecucion = ej.idPiscinaEjecucion
						--	inner join CICLOS_PRODUCCION cp on cp.Cod_Ciclo = ej.Cod_Ciclo
						--	where ej.Cod_Ciclo = 'COSTARICAI10.69' 
						--	AND CP.[Fecha_Siembra] <= DATEADD(DAY, 0, r.fechaRecepcion) 
						--	AND DATEADD(DAY, 0,  r.fechaRecepcion)  >= CP.[Fecha_Pesca]
		...
		--where nombreSector = 'COSTARICAI'



				END
				ELSE if not exists ( select top 1 1 from proRecepcionEspecieDetalle re 
								inner join EjecucionesPiscinaView ej on re.idPiscinaEjecucion = ej.idPiscinaEjecucion
					...
					BEGIN
						...
						select MAX(idRecepcion) from proRecepcionEspecie
						select MAX(idRecepcionDetalle) from proRecepcionEspecieDetalle
						...
						select 'ajskdbnflas', * from #InsertRecepTemp
		...
		--select * from proSecuencial where tabla in ('recepcionEspecie', 'recepcionEspecieDetalle')
		select 'FINAL propiscinaejecucion',* from EjecucionesPiscinaView where nombreSector = @SECTORFILTER and estado  in ('INI','EJE', 'PRE')  ORDER BY 2,5
		ROLLBACK TRAN 
		--commit TRAN 
	END TRY 
	BEGIN CATCH 
