	...
	select @PiscinaTemp = 'COSTARICAI10';
	...
	--WHERE cp.ciclo >= (select min(ciclo) from #EJECUCIONESTEMP p where p.keyPiscina = cp.Key_Piscina) AND Sector = 'CAPULSA' order by 1 -- @SECTORFILTER --and ej.ciclo is null
   ...
   --FILTRAMOS LAS PISCINAS A REGULARIZAR
		...
		from CICLOS_PRODUCCION cp
		left JOIN #EJECUCIONESTEMP ej on cp.Cod_Ciclo = ej.Cod_Ciclo 
		WHERE cp.ciclo >= (select min(ciclo) from #EJECUCIONESTEMP p where p.keyPiscina = cp.Key_Piscina) AND REPLACE(cp.Sector,' ','') = @SECTORFILTER and ej.ciclo is null
   ...
   --STEP 2: Query para creación de tabla de transferencia con homologaciones de piscinas con códigos de Insigne
	...
	WHERE R.KeyPiscina IN (@PiscinaTemp) --(SELECT FTP.Key_Piscina FROM #FILTERPISCINAKEYTEMP FTP) --r.Sector = @SECTORFILTER
	ORDER BY 1;
