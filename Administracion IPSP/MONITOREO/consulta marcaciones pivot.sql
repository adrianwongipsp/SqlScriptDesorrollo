DECLARE @fechaini DATE = '2024-04-04',
        @fechafin DATE = '2024-04-04',
        @IdTurno INT = NULL,
        @IdDescanso INT = 1;
...
WITH GENERAL AS (
    SELECT
        ...
        ev.Nombre AS Evento,
        CAST(uxm.Fecha AS DATE) AS FechaMarcacion,
...
SELECT 
    ...
    CASE
        ...
        THEN RIGHT('0' + CAST((DATEDIFF(MINUTE, HoraSalida, HoraEntrada) - @tiempoPermitido) / 60 AS VARCHAR), 2) + ':' +
             RIGHT('0' + CAST((DATEDIFF(MINUTE, HoraSalida, HoraEntrada) - @tiempoPermitido) % 60 AS VARCHAR), 2) + ':' +
             '00'
        ELSE ''
