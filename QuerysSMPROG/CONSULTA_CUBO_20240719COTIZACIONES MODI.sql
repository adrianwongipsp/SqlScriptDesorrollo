...
SELECT            
						 ...
						 CASE WHEN CC.REPUESTO = 1 THEN ISNULL(CD.REPUESTO,0) ELSE ISNULL(CD.DESC_SERVICIO,0) END AS DESCRIPCION_ITEM,
						 ...
						 ,
						 (select top 1 cf.NUM_FACTURA from BOD_CAB_FACT_INGRESO cf where cf.COD_COMPRA = OC.cod_compra and cf.TIPO_OC = OC.TIPO_OC AND CF.ESTADO NOT IN('A') and cf.COD_EMPRESA = DC.COD_EMPRESA) NUMERO_FACTURA,
						 --ISNULL(cf.NUM_FACTURA,'') NUMERO_FACTURA,
						 ISNULL(D.guia_remision,'')  NUMERO_GUIA_IZ,
						 case when cc.SERVICIO = 1 then CONVERT(varchar, OTC.FEC_FINALIZA,103) else null end FECHA_RECEPCION_SERVICIO,  CONVERT(varchar, OTC.FEC_FINALIZA,103),
						 case when cc.REPUESTO = 1 then CONVERT(varchar, d.Fecha_Documento,103) else null end FECHA_RECEPCION_REPUESTO,  CONVERT(varchar, d.Fecha_Documento,103),
						 DATEDIFF(day, (CASE WHEN CC.SERVICIO = 0 THEN OP.fecha_emision ELSE Rs.FEC_CREA END),(CASE WHEN CC.SERVICIO = 0 THEN d.Fecha_Documento ELSE OTC.FEC_FINALIZA END) ) TIEMPO_ENTREGA_REPUESTO_SERVICIO
						 ,right('0000000' + CAST(RS.NUM_REQUISICION AS VARCHAR),7),   right('0000000' + CAST(TRC.NUM_REQUISICION AS VARCHAR),7)
						 ,RS.NUM_REQUISICION, TRC.NUM_REQUISICION ,OC.cod_compra
...
FROM                     dbo.TSM_COTIZACION_CAB AS CC INNER JOIN
       ...
       ----                  dbo.TSM_REQ_SERVICIO_CAB AS RS ON RS.COD_REQUISICION = CDS.COD_REQUISICION AND CASE WHEN CC.SERVICIO = 1 THEN 1 ELSE 0 END = 1
						 ...
						 LEFT OUTER JOIN BOD_CAB_MOVIMIENTO D on D.Segun_Documento = CASE WHEN CC.SERVICIO = 1 THEN  right('0000000' + CAST(RS.NUM_REQUISICION AS VARCHAR),7)  ELSE   right('0000000' + CAST(TRC.NUM_REQUISICION AS VARCHAR),7) END and D.tipo_movimiento = 'I' --ESTA LINEA ES
